    ...
    for (const file of files) {
      ...
      if (stats.isFile()) {
        ...
        try {
          ...
          sha = data.sha; // Get the blob SHA if the file exists
        } catch (error) {
          // File does not exist; no need to set the sha
        }
        ...
        await octokit.repos.createOrUpdateFileContents({
          ...
          content,
          branch: newBranch,
          sha, // This will be undefined if the file does not exist, which is fine for creation
        });
      }
    }
